...
/**
 * "International Road Journey" — the dedicated visual identity for Saudi ->
 * GCC cross-border route pages. Distinct from every domestic route shell
 * (RouteJourneyView, AirportRouteView, CityAirportRouteView): the signature
 * features here are an explicit departure -> border -> arrival timeline
 * placed high on the page, a dedicated "what we handle vs what the border
 * controls" split panel (section 13 of the brief), and a document-prep
 * advisory panel — none of which apply to a same-country transfer. RTL-safe
 * (logical ps-/pe-, rtl:rotate-180 arrows), reused as-is for Arabic pages.
 */

export interface GccFact {
  label: string;
  value: string;
  emphasis?: boolean;
}

export interface GccStep {
  label: string;
  detail?: string;
}
...
export interface GccComparisonRow {
  criterion: string;
  road: string;
  flight: string;
}

export interface GccLinkItem {
  label: string;
  distance?: string;
  duration?: string;
  href: string;
...
export interface GccRouteViewProps {
  eyebrow: string;
  h1: string;
  dek: string;
  heroImage: string;
  ...
  whoSuits: GccScenario[];

  comparisonHeading?: string;
  comparisonIntro?: string;
  comparison?: GccComparisonRow[];

  checklistHeading: string;
  checklist: string[];

  bookingHeading: string;
  bookingSteps: GccStep[];
  ...
  relatedRoutes: GccLinkItem[];

  hubLinksHeading?: string;
  hubLinks: GccLinkItem[];
...
export function GccRouteView({
  ...
  return (
    <>
      {/* Hero — Saudi origin -> GCC country badge. */}
        ...
        <div className="relative mx-auto max-w-[1200px] px-4 pb-10 pt-32 sm:px-6 sm:pt-36 lg:px-8 lg:pt-40">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3 text-sm font-medium text-white/80">
            <span className="inline-flex items-center gap-1.5 rounded-full border border-white/25 bg-white/10 px-3.5 py-1.5">
              <MapPin className="size-3.5" /> {originLabel}, Saudi Arabia
            </span>
              ...
              <span
                ...
                className={`inline-flex items-center gap-2 rounded-lg border px-4 py-2 text-sm font-medium ${
                  f.emphasis
                    ...
                    : "border-hairline bg-sand/30 text-ink"
                }`}
              >
      ...
      {/* Journey timeline — the signature end-to-end feature. */}
      ...
      {/* Route-specific introduction / road journey. */}
      <section className="bg-white py-14 sm:py-20">
        <div className="mx-auto max-w-[1200px] px-4 sm:px-6 lg:px-8">
          <h2 className="text-xl font-bold tracking-tight text-ink sm:text-2xl">{introHeading}</h2>
      ...
      {/* Border crossing — a major section. */}
